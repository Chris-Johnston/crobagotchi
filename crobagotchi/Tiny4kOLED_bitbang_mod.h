...
static bool datacute_write_bitbang(uint8_t byte) {
	...
	{
		if ((byte << i) & 0x80) {
			PORTB |= (1 << BITBANG_SDA);//digitalWrite(SDA, HIGH);
		}
		else {
			PORTB &= ~(1 << BITBANG_SDA);//digitalWrite(SDA, LOW);
		}
		PORTB |= (1 << BITBANG_SCL);//digitalWrite(SCL, HIGH);
		PORTB &= ~(1 << BITBANG_SCL);//digitalWrite(SCL, LOW);
	}
...
static void tiny4koled_begin_bitbang(void) {
	pinMode(BITBANG_SCL, OUTPUT);
	pinMode(BITBANG_SDA, OUTPUT);
}
